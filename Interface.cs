    ...
    class Interface : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            if (!hidden && !disabled)
            {
                if (showMainWindow)
                {
                    MainWindowRect = GUILayout.Window(mainGuid, MainWindowRect, MainGUI, (Localizer.Format("#LOC_PR_MainGUI")));
                }
                if (showBodyWindow)
                {
                    BodyWindowRect = GUILayout.Window(bodyGuid, BodyWindowRect, BodyGUI, (Localizer.Format("#LOC_PR_BodyGUI")));
                }
                if (showConfigWindow)
                {
                    ConfigWindowRect = GUILayout.Window(configGuid, ConfigWindowRect, ConfigGUI, (Localizer.Format("#LOC_PR_ConfigGUI")));
                }
            }
        }
        void OnShowUI()
        ...
        private void ConfigGUI(int windowID)
        {
            ...
            {
                if (GUILayout.Button((Localizer.Format("#LOC_PR_ConfigGUI_Button3")), GUILayout.ExpandWidth(true)))
                {
                    visibilityMode = 3;
                    CreateBlizzyToolbar();
                    DeleteStockToolbar();
