    ...
    { 
        ...
        void OnGUI()
        {
            if (!hidden && !disabled)
            {
                if (showMainWindow)
                {
                    MainWindowRect = GUILayout.Window(mainGuid, MainWindowRect, MainGUI, "旋转维持");
                }
                if (showBodyWindow)
                {
                    BodyWindowRect = GUILayout.Window(bodyGuid, BodyWindowRect, BodyGUI, "选择界面");
                }
                if (showConfigWindow)
                {
                    ConfigWindowRect = GUILayout.Window(configGuid, ConfigWindowRect, ConfigGUI, "配置");
                }
            }
        }
        void OnShowUI()
        ...
        private void MainGUI(int windowID)
        {
            ...
            if (MainWindowActive)
            {
                if (activeVessel.IsControllable)
                {
                    ...
                    if (GUILayout.Button("旋转", GUILayout.ExpandWidth(true)))
                    {
                        mode = 1;
                    }
                    ...
                    if (GUILayout.Button("动力", GUILayout.ExpandWidth(true)))
                    {
                        mode = 2;
                    }
                    ...
                    if (mode == 1)
                    {
                        if (GUILayout.Button("相对旋转", GUILayout.ExpandWidth(true)))
                        {
                        ...
                        if (v.reference != null)
                        {
                            ...
                            {
                                GUILayout.Label("参考: 自动 (" + v.reference.GetName() + ")");
                            }
                            else
                                GUILayout.Label("参考: " + v.reference.GetName());
                        }
                        else
                        {
                            GUILayout.Label("参考: 无");
                        }
        ...
        private void BodyGUI(int windowID)
        {
            ...
            GUILayout.EndHorizontal();
            GUILayout.Label("在地图中选中目标 \n作为你的参考");
            if (GUILayout.Button("选择", GUILayout.ExpandWidth(true)))
            {
            ...
            if (GUILayout.Button("取消选择", GUILayout.ExpandWidth(true)))
            {
                v.reference = null;
                v.dynamicReference = false;
            }
            ...
            if (GUILayout.Button("自动", GUILayout.ExpandWidth(true)))
            {
        ...
        private void ConfigGUI(int windowID)
        {
            ...
            if (GUILayout.Button("总是可见的", GUILayout.ExpandWidth(true)))
            {
                visibilityMode = 1;
                DeleteBlizzyToolbar();
                DeleteStockToolbar();
            ...
            if (GUILayout.Button("常备的工具栏", GUILayout.ExpandWidth(true)))
            {
                visibilityMode = 2;
                CreateStockToolbar();
                DeleteBlizzyToolbar();
            ...
            {
                if (GUILayout.Button("Blizzy's 工具栏", GUILayout.ExpandWidth(true)))
                {
                    visibilityMode = 3;
                    CreateBlizzyToolbar();
                    DeleteStockToolbar();
            ...
            if (GUILayout.Button("无", GUILayout.ExpandWidth(true)))
            {
